        ...
        <div class="row widget-container q-pa-sm">
          ...
          <div class="col-12 col-md-4 q-pa-sm">
            ...
            dark
              ...
              v-model="year"
              label="Năm"
              @keyup="load(week, year)"
            >
              <template v-slot:append>
                <q-icon name="next_week" color="amber" />
              </template>
            </q-input>
          </div>
          <div class="col-12 col-md-4 q-pa-sm">
          ...
          <div class="q-pa-md">
            ...
            >
              <template v-slot:body="props">
                <q-tr :props="props">
                  <q-td v-if="!props.row.endPO" key="number" :props="props">
                    {{ props.row.number }}
                  </q-td>
                  <q-td v-else class="text-amber" key="number" :props="props">
                    {{ props.row.number }}
                  </q-td>
                  <q-td v-if="!props.row.endPO" key="status" :props="props">
                    {{  "Đang chạy" }}
                  </q-td>
                  <q-td v-else class="text-amber" key="status" :props="props">
                    {{  "Đã đóng" }}
                  </q-td>
...
export default {
  ...
  data() {
    return {
      ...
      deptSelectedRowKeys: [],
      departments: [],
      factory: { label: "NM CBG Thuận Hưng", factoryId: "100000", code: "TH" },
      factory1:null,
      factories: [
        {
  ...
  created() {
    ...
    if(this.userInfo.factoryId ==100000){
      ...
      this.count = false;
            ...
            console.log('user',this.userInfo.factoryId)

      this.factory = this.factories.find(ft=>ft.factoryId == this.userInfo.factoryId)
  ...
  methods: {
    ...
    async deletePO(payload) {
      const data = await this.$store.dispatch("production/DELETE_PO", payload);
