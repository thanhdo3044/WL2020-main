  ...
  <q-page class="q-pa-md">
    ...
    <div class="row justify-center">
      <div class="col-12 col-md-6 q-ma-md">
        ...
        <div class="row">
          <div class="col-12">
            <q-select style="font-family:'Time New Roman';font-size:16px;text-align:center" v-model="sourceSelect" behavior="dialog" :options="sources" @input="changeSource" label="Bộ phận tạo pallet :" />
          </div>
        </div>
        ...
        <div class="row" >
          <div class="col-12">
            <q-select style="font-family:'Time New Roman';font-size:16px" v-model="palletTypeSelect" behavior="dialog"
             :options="palletTypes" label="Loại pallet :" />
          </div>
        </div>
          ...
          <div class="col-12 q-ma-sm" >
            <q-card class="my-card">
              ...
              <q-card-section>
                ...
                <br>
                <q-input v-model="this.maxQuantityAvalible" type="text" label="Số lượng vật liệu đã nhận " :disable="true" />
                <br>
...
import {isObjectEmpty, showNotifySuccess} from '../../utils/utils'
...
export default {
    ...
    created(){
      ...
      let loadPalletTypes = new  Promise((resolve,reject)=>{
        let myTypeOfPackageTypeId = this.$store.getters["base/myStepInfor"].packageTypeGroupId ||  this.$store.getters["base/myXuongInfor"].packageTypeGroupId || this.$store.getters["base/myFactoryInfor"].packageTypeGroupId
    ...
    methods:{
      ...
      async loadProdInLSX(prodOrderId){
        ...
        if (this.sourceSelect) {
          if (this.currentsData.findIndex(d=>{return d.departmentId===this.sourceSelect.value})!==-1) {
            ...
            if (currIndex>0) {
              this.previousCurrent = this.currentsData[currIndex-1]
              let qty = this.previousCurrent.quantity-this.current.quantity
              this.maxQuantityAvalible = qty > 0 ? qty : 0
              cItem.quantity = this.maxQuantityAvalible
            }else {
               this.maxQuantityAvalible = dataMaterial.data.products[0].quantity
            }
            ...
            this.current =[]
            this.$q.notify({
              message:'Lệnh sản xuất này chưa có sản phẩm !'
            })
          }
        }
        this.$q.loading.hide()
      ...
      createPallet(){
        ...
        if (this.itemsSelected.length===1) {
          if (this.itemsSelected[0].quantity> this.maxQuantityAvalible) {
            this.$q.notify({
