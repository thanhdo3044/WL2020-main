...
export default {
    ...
    methods: {
        ...
        async approval(appStatus){
            ...
            let data = await this.$store.dispatch('wood/APPROVAL_PRICE',payload)
            if (data.meta.success===true) {
                this.$q.notify({
                    message:'Phê duyệt thành công !',
                    color:'green'
                })
                this.$router.push('/wood-price-list')
            }else {
                this.$q.notify({
        ...
        async initData(id){
            let data = await this.$store.dispatch('wood/GET_PRICE_BY_REQID',id)
