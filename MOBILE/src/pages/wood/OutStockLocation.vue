...
<!--         
        ...
        <div class="row">
            <div class="col-12">
               <q-select  style="font-family:'Time New Roman';font-size:16px;text-align:center" @input="loadItemInReceipt" v-model="receipt" behavior="dialog" :options="receipts" label="Số phiếu" />
            </div>
        </div>
         <div class="row">
          <div class="col-12">
        ...
        <!-- <div class="row q-mt-md">
          <div class="col-12">
             <DxDataGrid
              ...
              :show-borders="true"
              key-expr="id"
              no-data-text="Nhấn dấu + phía trên để thêm mã xuất"
        ...
        <div class="row justify-center" v-if="typeScreen==='say'">
          <div class="col-9">
            <q-select v-model="item" :options="items"  use-input input-debounce="0" @filter="searchItem" behavior="dialog" label="Kích thước" outlined />
          </div>
          <div class="col-3">
            <q-btn color="cyan" class=" full-height q-ml-sm"  label="Chọn" @click="chonQuiCach" />
          </div>
        </div>
        <div class="row justify-center q-mt-md">
          <div class="col-12 q-ma-sm" v-for="(item,index) in itemsSelected" v-bind:key="item.id">
            <q-card class="my-card">
              ...
              <q-card-section>
                <q-input readonly v-model="item.inputQty" v-if="typeScreen!=='say'" type="text" label="Số lượng nhập" />
                <br v-if="typeScreen!=='say'">
...
import {isObjectEmpty, formatNumber} from '../../utils/utils'
...
export default {
    ...
    computed:{
      wathValidate(){
        return `${this.sourceSelect}|${this.destinationSelect}|${this.itemsSelected}|${this.receipt}`
      }
    },
    watch:{
      wathValidate(newVal,oldVal){
        const [sSelect, dSelect,iSelect,rSelect] = newVal.split('|');
        if (sSelect!=='null'&&dSelect!=='null'&&rSelect!=='null'&&iSelect!=='null') {
         this.validate=true
        }else {
          this.validate=false
        }
    ...
    created(){
      let loadReceipt = new Promise(async(resolve,reject)=>{
        ...
        if (myWarehouseId) {
          let dataReceipt = await this.$store.dispatch('wood/GET_RECEIPTS',payload)
      ...
      let loadItems = new Promise(async(resolve,reject)=>{
        ...
        let dataItem = await this.$store.dispatch('production/GET_ITEMS',payload)        
        dataItem.forEach(it=>{
          this.items.push({
            value:it.id,
            label: it.name
      ...
      Promise.all([loadItems,loadReceipt]).then(values=>{
    ...
    methods:{
      ...
      // },
       async loadItemInReceipt(val){
         const typeScreen = this.$route.query.type
        if (typeScreen!=='say') {
          const id = val.value
          const data =  await this.$store.dispatch('wood/GET_DETAIL_RECEIPT_BY_RECEIPT',id)
          this.itemsInReceipt = data.data
          this.itemsInReceipt.forEach(i=>{
            if (this.itemsSelected.findIndex(f=>{return f.id===i.ID})===-1) {
              this.itemsSelected.push({
                id:i.ID,
                name:i.ITEM_NAME,
                quantity:'',
                inputQty:formatNumber(i.PACKAGE_QUANTITY*i.PCS_PER_PACKAGE)
              })
            }
          })
        }
      ...
      onEditorPreparing(e){
        if (e.dataField === 'id' || e.dataField === 'height'||e.dataField === 'length'||e.dataField === 'width') {
      ...
      chonQuiCach(){
        if (this.item) {
          ...
          this.$q.notify({
            message:'Vui Lòng chọn sản phẩm trước !'
          })
        }
      },
      blurMaLoGo(){
