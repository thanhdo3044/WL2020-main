<template>
  <q-page class="q-pa-sm">
      ...
      <div class="row justify-center q-mt-sm">
          <div class="col-12 col-sm-6">
              <q-btn @click="confirm=true" class="full-width" color="primary" glossy label="Nhập kho" />
          </div>
      </div>
...
export default {
    ...
    methods: {
        async initData(){
            return Promise.all([this.loadWoodTypes(),this.loadInputType(),this.loadItems(),this.loadDType()])
        ...
        async submit(){
            if (this.validate) {
                ...
                let data = await this.$store.dispatch('wood/CREATE_DETAIL_RECEIPT',payload)
