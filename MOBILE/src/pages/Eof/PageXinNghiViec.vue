<template>
  <q-page>
    ...
    <order-approved class="q-mt-lg" v-if="disable" :metaValue="metaValue" :order="order"/>
  ...
  created() {
    if (this.$route.query.id) {
      this.getOneOrder(this.$route.query.id).then(res => {
        console.log('đấ',res.data.data);
        if(res.data.data.accountId != this.userInfo.id){
          this.$router.push({
        path: "/"
      });
        }
        if (res.data.data.Requests.length > 0) this.disable = true;
