    ...
    <div class="row justify-center">
      <div class="col-12 col-sm-6 q-ma-sm">
        <q-card class="q-pa-md">
          <div class="row">
            <div class="col-12">
              <p class="prodTextTitle">Nhập số pallet để xuất</p>
            </div>
          </div>
          <div class="row">
            <div class="col-8 q-pa-xs">
              <q-input @clear="clearCode" clearable outlined v-model.number="palletCode" label="Số pallet" />
            </div>
            ...
            if (this.mySources.findIndex(s=>{return s.id===palletToDestiantionId.stepNextId})!==-1) {
              ...
              if (packagePallet.verifyBy) { 
                ...
                this.$store.dispatch('production/ACTION_GET_DESTINATION_BY_ITEM',payloadGetDes).then(resp=>{
                  ...
                  if (this.$store.getters["production/myDestinations"].length>0) {
                    this.$store.getters["production/myDestinations"].forEach(element => {
                ...
                setTimeout(() => {
                  this.$router.push('/funiture-dashboard?code='+this.pallet.code+'&path=funiture-stockout-pallet?code='+this.pallet.code)
                }, 1000);
              }
            }else {
        ...
        this.$store.dispatch('production/ACTION_STEP_PALLET',payload).then(res=>{
            if (res.meta.success===true) {
                ...
                if (parseFloat(this.failQty)>0) {
                  ...
                  const myFactoryId = this.$store.state.production.userInfo.factory.id
