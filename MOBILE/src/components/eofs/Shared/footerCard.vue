  ...
  <div>
    ...
    >
      ...
      <q-card-section v-else-if="request.status == 'Đã ký'">
        ...
        <q-btn
        class="text-h6 text-white shadow-10"
          @click="promptToDelete(request)"
          color="red"
          icon="clear"
          label="Xóa khẩn cấp"
      ...
      <q-card-section v-else-if="active1">
        <q-btn class="text-h6 bg-red text-white shadow-10"
          >Quá hạn phê duyệt</q-btn
        >
        <!-- <div class="text-subtitle2">{{ request.updatedAt | luc }}</div> -->
      </q-card-section>
    ...
    <div
      v-else-if="
        (userInfo.account == 'BVTH' ||
          userInfo.account == 'BVTB' ||
          userInfo.account == 'BVYS' ||
          userInfo.account == 'BVYQM' ||
          userInfo.account == 'BVCH' ||
          userInfo.account == 'BVVF' ||
          userInfo.account == 'BVYNH') &&
        request.slug != 'xac-nhan-cong'
      "
    >
      <div v-if="request.status == 'Không' || request.status == 'Chưa ký'">
        <q-btn
...
export default {
  props: ["order", "nextId", "nextId2", "nextId3", "active1"],
  computed: {
  ...
  methods: {
    ...
    async changeAuthority() {
      ...
      console.log("changeAuthority", moment().add(2, "days").calendar());
    ...
    async sendMessagesSuccess(nameActive, nameSuccess, dataSuccess) {
      ...
      //Gửi thông báo email cho cấp tiếp theo
      let baseUrl2 = null;
      let nextId = null;
      if (this.nextId === undefined) {
        ...
        nextId = 0;
      } else nextId = this.nextId.accountId;
      ...
      if (dataProfile2 === undefined) {
        ...
        dataProfile3 = 0;
      } else if (dataProfile2.notificationMail) {
        dataProfile3 = 1;
      }
      if (dataProfile3 == 1) {
      ...
      await dataFCMStep.data.data.forEach(async (element) => {
        if (element.tokenFCM != "undefined") {
          ...
          let baseUrl = null;
          if (process.env.PROD) {
            baseUrl = `https://app.woodsland.com.vn:1001/#/yeu-cau-duyet?id=${this.request.orderId}`;
          } else {
            baseUrl = `http://localhost:8080/#/yeu-cau-duyet?id=${this.request.orderId}`;
          }
          let payloadFCM = {
            // lưu thông tin gửi thông báo FCM
            baseUrl: baseUrl,
            ...
            messageId: dataSendFCM.data.results[0].message_id,
            dataBody: ` Có phiếu gửi tới - ${this.nextId.title} - người gửi : ${
              this.getAccountById(this.request.accountId).lastName
            } - mã NV : ${
              this.getAccountById(this.request.accountId).account
            } - phòng ban : ${departmentName.NAME} `,
          };
          await this.sendNotificationMail(payloadFCM); //lưu thông tin FCM
        }
    ...
    completedToRequest(request) {
      this.$q
        ...
        .onOk(async () => {
          if (
            this.request.slug == "xac-nhan-cong" &&
            (this.order.OrderMeta[0].fullName
              ? this.order.OrderMeta[0].date
              : this.order.OrderMeta[1].date) < moment().format("DD-MM-YYYY")
          ) {
            this.$q.notify({
              message: "Phiếu đã quá hạn phê duyệt !!",
              color: "red",
            });
            console.log(
              this.request,
          ...
          if (
            ...
            await this.completedRequest2(payload);
            this.$router.push({
              path: "/eof-approval",
            });
            this.$root.$emit("activeRequests");
      ...
      this.$q
        ...
        .onOk(async () => {
          if (
            ...
            this.nextId2.forEach(
              async (e) => await this.deleteRequestById(e.id)
            );
            this.$router.push({
              path: "/eof-approval",
            });
  ...
  async created() {
    ...
    console.log("nextId", this.active1);
    // await this.loadData();
  },
};
</script>
