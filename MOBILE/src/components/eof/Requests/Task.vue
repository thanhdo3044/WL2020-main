  ...
  <div>
    ...
    >
      ...
      <q-item-section>
        ...
        <q-item-label caption lines="3"
          ...
          <q-badge
            class="q-ml-sm"
            color="red"
            v-if="task.priorityLevel == true"
            :label="'Ưu tiên'"
          />
          <q-badge
            v-if="
              task.ProposalForm.parentId == 1024 ||
              task.ProposalForm.parentId == 1028
            "
            class="q-ml-sm"
            color="red"
            :label="task.activeSeen == 1 ? 'Phòng vật tư: đã xem' : 'Phòng vật tư: chưa xem'"
          />
          <!-- <q-badge
            class="q-ml-sm"
            color="teal"
            v-if="
              (task.slug == 'xac-nhan-cong' &&
                (task.OrderMeta[0].fullName
                  ? task.OrderMeta[0].date
                  : task.OrderMeta[1].date) >= moment().format('DD-MM-YYYY')) ||
              !(
                (task.slug == 'xin-nghi-viec' || task.slug == 'xin-ra-cong') &&
                moment(
                  moment(this.task.OrderMeta[0].dateStart, 'DD-MM-YYYY').format(
                    'YYYY-MM-DD'
                  )
                ...
                task.status != 'Đã được duyệt'
              )
            "
            :label="task.activeSeen == 1 ? ' đã xem' : 'chưa xem'"
          /> -->
          <q-badge
            class="q-ml-sm"
            color="red"
      ...
      <q-item-section side>
        <div class="row">
          <q-btn
            v-if="task.status != 'Đã được duyệt'"
