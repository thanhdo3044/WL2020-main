<template>
    <div  class="q-pa-lg" style="max-width: 100%; width: 100%;">
      <q-select
        class=""
        filled
        @input="chonAccount"
      ...
      <q-calendar 
        ...
        view="month"
        locale="en-us"
        style="height: 100%; border: 1px solid #ccc; font-size :12px"
        animated
      ...
      > {{timestamp}}
        <template #day="{ timestamp }">
          <template v-for="(event, index) in getEvents(timestamp.date)">
            ...
            >
              <q-icon v-if="event.icon" :name="event.icon" class="q-mr-xs"></q-icon><span class="ellipsis">Thời gian bắt đầu {{event.date}} / {{event.time}} </span>
            </q-badge>
          </template>
        </template>
      </q-calendar>
      <DialogAddCalendarWork  v-on:childToParent="onChildClick" :dialog="dialogAdd" :selectedDate="selectedDate" v-if="dialogAdd"></DialogAddCalendarWork>
                ...
                <q-item v-for="dataActives in dataActive.data" :key="dataActives.id">
                    <q-card class="">
                        ...
                        <div class="float-right">
                            <q-icon class="bg-primary q-ma-sm q-pa-sm" style="font-size: 1.5em;border-radius:50%"  name="delete" flat label="Thoát" @click="delCalendarDetail(dataActives.id)" color="white"  />
...
<script>
import DialogAddCalendarWork from 'components/eof/CalendarWork/DialogAddCalendarWork.vue';
import DialogEditCalendarWork from 'components/eof/CalendarWork/DialogEditCalendarWork.vue';
...
export default {
    ...
    onChildClick (value) {
      ...
      console.log('this.dialog',this.dialogAdd)
    },
    onChildClick2 (value) {
      this.events = value
      console.log('this.dialog',this.events)
    },
    onModelChanged (date) {
      this.dialog2=false;
    ...
    async onClickDate2 (data) {
      this.selectedDate = JSON.stringify(data.scope.timestamp.year) 
        ...
        await axios({
            method:'POST',
            url:"http://localhost:1010/api/v1/calendarWorkSearch",
            data:{
    ...
    async delCalendarDetail(id){
        await axios
                .delete("http://localhost:1010/api/v1/calendarWork/" + id)
                .then(res => {
                    if(res){
                       this.$q.notify({
    ...
    async editCalendarDetail(sumPeople,description,startDate,endDate,id,title,numberPhone,location){
        ...
        // async loadData() {
        //     await axios
        //         .get("http://localhost:1010/api/v1/calendarWork")
        //         .then(response => this.calendarWork = response.data);
