          ...
          <q-card-section class="q-pt-none">
            ...
            /> -->
              ...
              <q-card-section class="q-ql-none" >
                <q-select  class="" filled v-model="model" :options="options" label="Tùy chọn hiển thị" />
...
export default{
    ...
    props:[
        "selectedDate","dialog","endDate","numberPhone","title","location","id","description"
        ,"sumPeople","startDate"
    ],
    data () {
        return {
    ...
    //     this.$emit('name',this.events  );
    // },
    async delAccount(){
      this.getRandomColor(8);
       await axios({
         method: "PUT",
          url: "http://localhost:1010/api/v1/calendarWork/" + this.id,
          data: {
      ...
      .then(res => {
        ...
        if(err){
          this.$q.notify({
              message: "Sửa thất bại - kiểm tra lại thông tin ",
              color: "purple",
              icon: "announcement"
            });
        }
    ...
    getName (event) {
      ...
      this.$emit('getName', this.dialogEdit)
    }
  },
   computed: {
