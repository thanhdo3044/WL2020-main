  ...
  <q-page>
    <q-banner class="bg-primary text-orange">
      <div class="text-h6 orange">
        <span color="orange">Xuất tồn kho gỗ sau sấy</span>
      </div>
    </q-banner>
    <div>
...
export default {
  data() {
    return {
      columns: [
        ...
        { name: "domicile", label: "Lệnh sản xuất", field: "poId" },
        { name: "date_of_birth1", label: "Khối lượng", field: "quantity" },
        { name: "date_of_birth3", label: "Xuất đến", field: "destination" },
  ...
  methods: {
    ...
    async updateITEMPACKAGE(PACKAGE_ID, xuatton) {
      const payload = {
        PACKAGE_ID,
        quantity: xuatton.quantity,
        itemId: xuatton.itemId,
      };
      console.log(payload);
    ...
    async onClick() {
      ...
      if (this.selected) {
        if (this.resultData.length > 0) {
          ...
          await dataSlice.forEach(async (item) => {
            console.log(item)
          ...
          let result = [];
          let dataPk = [...this.resultData]

          dataPk.forEach(async (e) => {
            e.SOURCE_ID = 102484;
            e.quantity = Math.abs(e.quantity);
            e.itemId = parseInt(e.itemId);
            e.DESTINATION_ID = parseInt(e.destination);
            e.PO = e.poId.split(" ")[0].toString();
            e.CREATE_BY = userId;
          ...
          dataPk.forEach(async (e) => {
            ...
            const data1 = this.updateITEMPACKAGE(data[0].ID, {
              quantity: e.quantity,
              itemId: e.itemId,
            });
          });
          ...
          await results.forEach((e) => {
            if (e.QUANTITY) {
              this.resultData.push({
                ...
                nguonPhoi: e.NGUON_PHOI ? e.NGUON_PHOI : null,
                product_id: e.PRODUCT_ID ? e.PRODUCT_ID : null,
                detailed_id: e.DETAILED_ID ? e.DETAILED_ID : null,
                itemId: e.ITEM_ID ? e.ITEM_ID : null,
                destination: e.STEP_ID ? e.STEP_ID : null,
                description: e.DESCRIPTION ? e.DESCRIPTION : null,
                selection: e.SELECTION ? e.SELECTION : null,
                poId: e.PO_ID ? e.PO_ID : null,
