  ...
  <q-page class="q-pa-sm">
    <div class="row" style="background-color: #daf8e1">
      ...
      <div class="col-5 text-blue text-bold" align="right">
        ...
        <div class="col-1 text-blue text-bold" align="right">
        ...
        <div class="q-pa-md doc-container" style="margin-top: 30px">
          <div class="row justify-end">
            <div v-if="dataSourcedat.length>0" class="col-4 text-center">
              <p  style="margin: 0px">
              Ngày {{ dataSource[0].ngay }} tháng
                {{dataSource[0].thang}} năm {{ dataSource[0].nam }}
              </p>
              <b>Phòng cơ điện</b><br />

              <img
                v-if="dataSource[0].CREATE_BY"
                v-bind:src="'/statics/' + dataSource[0].CREATE_BY + '.png'"
            ...
            <div v-if="dataSourcedat.length>0" class="col-4 text-center">
             Ngày {{ dataSource[0].ngaysx }} tháng
              {{ dataSource[0].thangsx }} năm {{ dataSource[0].namsx }} <br />
              <b>Sản Xuất</b><br />

              <img
                v-if="dataSource[0].SX_VERIFY_BY"
                v-bind:src="'/statics/' + dataSource[0].SX_VERIFY_BY + '.png'"
                alt="Không có chữ kí số"
                width="180"
                height="auto"
              /><br /><br /><br />{{ dataSource[0].sx }}<br /><br />
            </div>
          </div>
        </div>
      <DialogSelectDate
  ...
  methods: {
    ...
    async load() {
      ...
      const data = await this.$store.dispatch(
        "machine/GET_REPORT_BAO_DUONG_CAP_3",
        payload
      );
        const data2 = await this.$store.dispatch(
        "machine/GET_REPORT_BAO_DUONG_CAP_3_LOI",
        payload
      );
