...
import TableRequest from '../../components/NLVan/TableRequestPrice'
...
export default {
  ...
  methods: {
    async sendRequest(){
      ...
      let data = await this.$store.dispatch('nlvan/CREATE_REQ_PRICE',payload)
    ...
    async loadCurrentPrice () {
      let payload = {
        moduleCode: this.$route.query.type,
        regionId: this.region.value
      }
    ...
    async loadRegion(){
      let data = await this.$store.dispatch('nlvan/GET_REGION',{module:this.$route.query.type})
    ...
    loadMyRequest(){
      return new Promise(async(resolve,reject)=>{
        ...
        let data = await this.$store.dispatch('nlvan/GET_MY_REQUEST',myId)
    ...
    loadwType() {
      // keo, keo F1
      return new Promise(async (resolve, reject) => {
        let data = await this.$store.dispatch("nlvan/GET_WOOD_TYPE");
