<template>
  <q-page class="q-pa-sm">
    ...
    <div class="row justify-end q-mt-md print-hide">
      <div class="col-2">
        <q-btn @click="showConfirmReject = true" color="amber" style="font-family:'Time New Roman'" glossy label="Từ chối" />
      </div>
      <div class="col-2">
    ...
    <q-dialog v-model="showConfirmReject" persistent>
      <q-card>
        <q-card-section class="row items-center">
          <span class="q-ml-sm top-title">Xác nhận từ chối biên bản !.</span>
        </q-card-section>
        <q-card-section>
          - Biên bản sẽ được mở lại cho bộ phận QC và Kho chỉnh sửa dữ liệu !
        </q-card-section>
        <q-card-actions align="right">
          <q-btn flat label="Hủy" color="primary" v-close-popup />
          <q-btn flat label="Đồng ý" @click="reject" color="primary" v-close-popup />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </q-page>
...
import { sortParent,roundNumber,formatNumber, formatDateVN, showNotifySuccess, showNotifyError } from "../../ultils";
export default {
  data() {
    return {
      receipt: {},
  ...
  methods: {
    async reject(){
      const data  = await this.$store.dispatch('nlvan/REJECT_BB',this.$route.params.id)
    ...
    async completed(){
      ...
      let data  = await this.$store.dispatch('nlvan/COMPLETED_PAYMENT',payload)
      this.$q.loading.hide()
    ...
    goManual(){
      ...
      this.$store.dispatch('nlvan/STORE_PAY_CALCUTALOR',payload)
