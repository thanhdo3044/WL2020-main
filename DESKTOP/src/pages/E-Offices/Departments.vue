<template>
  <q-page class="q-pa-md q-ma-sm">
    <q-card class="my-card">
      <q-card-section>
        <div class="row">
          <div class="col-12 top-title">Quản lý phòng ban</div>
        </div>
      </q-card-section>
      <q-card-section>
        <div>
          ...
          >
            ...
            >
              <DxLookup
                :data-source="listAccountInDepartment"
...
export default {
  ...
  methods: {
    ...
    onSelectionChanged(e) {
      ...
      this.iGroupOrder = this.groupDepartment.find(
        (u) => u.value == e.data.data.groupOrder
      );
      this.iFactory = this.factories.find(
        (u) => u.value == e.data.data.factoryId
      );
      this.id = e.data.data.id;
      this.iParentId = this.strDepartmentSelect.find(
    ...
    async updated() {
      ...
      if (this.iGroupOrder) {
        if (this.iGroupOrder.value == 0) {
           await this.$store.dispatch(
        "prod/DELETE_GROUP_DEPARTMENT",
        {departmentId: this.id}
      );
        }else{
          await this.addGroupDepartment(
            this.id,
            this.iGroupOrder.value,
            this.iGroupOrder.name,
            this.iGroupOrder.code
          );
        }
      }
    ...
    async addGroupDepartment(id, order, name, code) {
      const payload = {
        order: order,
        name: name,
        departmentId: id,
    ...
    async loadDepartment() {
      ...
      data.forEach((el) => {
        const itemNameConvert = `[${el.code}] - ${el.name} `;
        this.strDepartmentSelect.push({
    ...
    async loadUsers() {
      ...
      const data2 = await this.$store.dispatch("base/GET_ACCOUNT_IN_DEPARTMENT");
      data.data.forEach((el) => {
        const account = `[${el.account}] - ${el.lastName}`;
        this.users.push({
          label: account,
          value: el.id,
          name: el.lastName,
      ...
      this.listAccountInDepartment = data2.data;
    ...
    async loadGroup() {
      this.groups = [];
      const data = await this.$store.dispatch(
        "production/GET_NHOM_TIEU_THU_DIEN"
      );
      this.groups = data;
    },
  },
