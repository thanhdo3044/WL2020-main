  ...
  <q-page class="q-pa-md">
    <q-banner class="text-primary">
      ...
      <div class="text-h6 q-mt-lg">
        Tạo phiếu chỉ định cấp gỗ - {{ idDepartment.label }}
      </div>
    </q-banner>
    <div class="row">
      <q-select
    ...
    <q-card-section>
      ...
      <q-btn
        color="orange"
        icon="assignment_return"
        label="Cấp gỗ theo thanh có hệ số"
        @click="capgothanhTH"
      />
         &emsp; &emsp; &emsp;
          <q-btn
      ...
      <q-card class="my-card">
        ...
        <q-card-section>
          ...
          >
            ...
            <DxColumn data-field="paren" caption="Sẩn phảm" :group-index="0" />
            <DxColumn data-field="tensp" caption="Chi tiết" width="350" />
            <DxColumn data-field="NAME" caption="Nhóm Qui Cách" width="180">
            </DxColumn>
            <DxColumn
              data-field="PHOI"
            ...
            />
            <!-- <DxColumn caption="Tồn lựa phôi" alignment="center">
              <DxColumn
                caption="TH"
                data-field="TONTHKL"
                format="#,##0.####"
            ...
            <DxColumn data-field="BX" caption="Bổ sung phiếu lần" width="100" />
            <DxColumn data-field="MALOGO" caption="Mã lô gỗ" />
            <!-- <DxColumn
    ...
    <q-dialog v-model="confirmTrave" persistent>
      <q-card style="min-width: 400px">
        <q-card-section class="row items-center">
          <div class="row">
            <div class="col-12 top-title">Xác nhận tạo phiếu chỉ định</div>
          </div>
        </q-card-section>
...
export default {
  ...
  created() {
    ...
    Promise.all([this.loadGroupQuiCach(), this.loadItem(), this.loadGrid()]);
  ...
  methods: {
    ...
    async loadHistory() {
      ...
      const data = await this.$store.dispatch("nlg/GET_HE_SO_CAP_GO", payload);
    ...
    async bosung() {
      console.log("this.dataTrave", this.dataTrave);
 const payload = {
        number: this.number,
        solan:this.dataTrave[0].BX,
      };
      const data2 = await this.$store.dispatch(
        "nlg/GET_PHIEU_CHI_DINH_CHAN",
        payload
      );
 ...
 this.$q.dialog({
          html: true,
          title: "Tạo phiếu chỉ định",
          message: "bạn đã tạo phiếu chỉ định bổ sung" + " " + this.dataTrave[0].BX + "cho LSX: " + this.number + " rồi",
        });
      }
      else {
      let check = false;
      this.dataTrave.forEach((el) => {
        if (el.BX == "") {
          check = true;
        }
      });
      if (!check) {
        ...
        this.$q.dialog({
          html: true,
          title: "Chú ý!",
          message: `<span class="text-red">Vui lòng nhập số bổ sung ! </span>`,
          persistent: true,
        });
      }
      }
  ...
  const payloaddinhtuyen = {
    ...
    async capgothanh() {
      this.$router.push("/nlg-tao-phieu-chi-dinh-thanh/");
    },
    async capgothanhTH() {
      this.$router.push("/nlg-tao-phieu-chi-dinh-thanh-th/");
    },
    async capgoTP() {
      this.$router.push("/nlg-tao-phieu-chi-dinh-TP/");
    },
    ...
    calculateTongThanh(rowData) {
      rowData.klc = roundNumber(rowData.HESO * rowData.keHoach, 4);
