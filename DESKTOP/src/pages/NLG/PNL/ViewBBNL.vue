...
export default {
  ...
  methods: {
    ...
    async reject(){
      ...
      const data = await this.$store.dispatch('nlg/UPDATE_PHIEU_NHAP_KHO_NL',payload)
      if (data.meta.success) {
        showNotifySuccess()
         this.$router.push('/nlg-bb-NL')
      }else{
        showNotifyError()
      }
    },
    ...
    async loadPhieu() {
     const guid = this.$route.params.guid.replace('-','/');
      ...
      if (guid) {
        this.guid = guid;
        const data = await this.$store.dispatch("nlg/GET_DS_PHIEU_TT_NHAP_LAI", {
          guid: guid
        });
...
let  thetichQC1 = roundNumber((element1.KLKHO*element1.SOTHANH/element.SL_MAU)*hc_gapdoi1,4);
            tongthanhtien += roundNumber(element1.DONGIA_LOAI*thetichQC1 ,0);

            tongTheTichQCCP += roundNumber((element.KLKHO),4)*hc_gapdoi1;
       ...
       eldata+=`
            <tr >
              ...
              <td style="text-align:center;">${roundNumber(element.KLKHO,4)}</td>
              <td ></td>
              <td >${formatNumber(element.DONGIA_CTY)}</td>
         
              <td >${formatNumber(roundNumber(element.DONGIA_LOAI*roundNumber((element.KLKHO),4),0))}</span></td>
              <td>${formatNumber(roundNumber(element.DONGIA_LOAI,0))}</td>
          </tr>
          `
    ...
    //          thetichQC = roundNumber(element2.KLKHO*Tyle*hc_gapdoi,4);
    ...
    //           <td >${note}</td>
    //           <td >${formatNumber(element2.DONGIA_CTY)}</td>
    //           <td>${formatNumber(element2.DONGIA_LOAI)}</td>
    //           <td >${formatNumber(roundNumber(element2.DONGIA_LOAI*thetichQC,0))}</span></td>
    //           <td></td>
    //       </tr>
    //       `
    //      }
      ...
      this.tinhtien = data4.data.tinhtien[0];
        ...
        if (this.tinhtien.XUATHOADON.trim() =='Y')
            ...
            xuathoadon = `
                  ...
                  <td colspan="2" style="text-align:center;color:red" >${formatNumber(roundNumber((this.tinhtien.TONGTIENCHUATHUE/this.tinhtien.KLKHO),0))}</td>
                    `
             }
          else{
            xuathoadon = `
        ...
        eltotal=`<tr>
                  ...
                  <td style="text-align:center;"><span id="tongthetichnhap">${roundNumber(this.tinhtien.KLQC,4)}</span></td>   
                  <td  style="background-color:cadetblue;text-align:center;"><strong id="tongthetichkho">${roundNumber(this.tinhtien.KLKHO,4)}</strong></td> 
                  <td colspan="2" style="background-color:cadetblue;"><strong>Tổng cộng :</strong></td> 
                  <td><span style="color:red" id="tongcong">${formatNumber(roundNumber(this.tinhtien.TONGTIENCHUATHUE - this.tinhtien.HOTROKHAC,0))}</span></td> 
                  <td><span style="color:red" id="dongiatb">${formatNumber(roundNumber(this.tinhtien.DONGIATB,0))}</span></td> 
              </tr>
              <tr>
                  <td colspan="9" style="text-align:right;"><strong>Tổng thể tích các quy cách gỗ bị loại được thanh toán :</strong></td> 
                  <td style="text-align:center;">${0}</td>
                   <td colspan="3" style="text-align:right;color:red">Hỗ trợ khác :</td>
                  <td colspan="3" style="text-align:center;color:red" >${formatNumber(roundNumber(this.tinhtien.HOTROKHAC,0))}</td>
                 
              </tr>
              <tr>
                  <td colspan="9" style="text-align:right;"><strong>Tổng thể tích quy cách gỗ nhập được thanh toán :</strong></td> 
                  <td style="text-align:center;">${roundNumber(this.tinhtien.KLQC,4)}</td>
                 <td colspan="3" style="text-align:right;color:red">Tổng tiền chưa thuế :</td>
                  <td colspan="2" style="text-align:center;color:red" >${formatNumber(this.tinhtien.TONGTIENCHUATHUE)}</td>
              </tr>
              
              <tr>
                  <td colspan="9" style="text-align:right;"><strong>Tỷ lệ chính phẩm của lô hàng :</strong></td> 
                  <td style="text-align:center;">< 48%</td>
                  <td colspan="3" style="text-align:right;color:red">Thuế 8% :</td>
                  <td colspan="2" style="text-align:center;color:red" >${formatNumber(this.tinhtien.TONGTHANHTOAN - this.tinhtien.TONGTIENCHUATHUE)}</td>
              </tr>
              <tr>
                  <td colspan="10" style="text-align:right;"><strong></strong></td> 
                  <td colspan="3" style="text-align:right;color:red">Tổng thanh toán :</td>
                  <td colspan="2" style="text-align:center;color:red" >${formatNumber(roundNumber(this.tinhtien.TONGTHANHTOAN ,0))}</td>
              </tr>
             ${xuathoadon}
             `
