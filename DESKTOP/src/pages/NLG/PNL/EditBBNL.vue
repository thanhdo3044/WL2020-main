  ...
  methods: {
    async save(){
     // console.log(this.hotrokhac)
      ...
      const data = await this.$store.dispatch('nlg/COMPLETED_BBNL',payload)
    
      if (data.data) {
        showNotifySuccess()
       
          this.$router.push('/nlg-bb-tinh-tien-viewNL/'+this.phieunhapkho.sophieu.replace('/','-'))
    ...
    async reject(){
      const payload = {
        id:this.phieunhapkho.sophieu,
        delai:'Y',
        
       
      }
      const data = await this.$store.dispatch('nlg/HUY_PHIEU_NHAP_LAI',payload)
...
//       const bg2 = this.dupplicate6
     ...
     if (element.DAY < 24 && element.DAY >=17){
       giactycp = 1500000
     }
      if (element.DAY < 17 && element.DAY >=14){
       giactycp = 400000
     }
     if (this.phieunhapkho.sophieu.trim() == '2ML.5YS1'){
        giactycp = 400000
     }
      // else {giactycp = element.COST; }
       let tongthanhtien = roundNumber((giactycp*roundNumber((element.KLKHO),4)),0);
      ...
      //  else this.hotrokhac = 0;
         tongTheTichQC += roundNumber((element.KLKHO),4);
             ...
             if (this.phieunhapkho.mancc.trim() == 'TH' && element.codenhom.trim() == '100115' )
      {
       ...
       eldata+=`
            <tr style="${classBG}" >
              ...
              <td>${formatNumber(giactycp)}</td>
              <td >${formatNumber(roundNumber(giactycp*roundNumber((element.KLKHO),4),0))}</span></td>
              <td>${formatNumber(giactycp)}</td>
          </tr>
          `
       ...
       // console.log(this.tongthanhtoan)
        ...
        eltotal=`<tr>
              ...
              <tr>
                  ...
                  <td style="text-align:center;">${roundNumber(tongTheTichQC,4)}</td>
              ...
              <tr>
                  ...
                  <td colspan="5" style="text-align:right;color:red">Thuế 8% :</td>
