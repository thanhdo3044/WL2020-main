            ...
            />
             <template #edit-template="cell">
                <div>
                    ...
                    <span  v-if="cell.data.data.tinhtien.toString().trim()==='Y' && cell.data.data.ALLOWHACAP.toString().trim()==='N'" @click="goAdd(cell.data.data.sophieu)" class="fake-link">Tính tiền</span>
...
export default {
    ...
    methods:{
        async load(){
            const data = await this.$store.dispatch('nlg/GET_DS_PHIEU_NHAP_KHO',{kho:'ALL'})
            this.dataSource = data.data
        },
        view(guid){
           let routeData = this.$router.resolve('/nlg-bb-tinh-tien-view/'+guid.replace('/','-'))
           window.open(routeData.href, '_blank');
        },
        goAdd(guid){
            //console.log(data);
            //const sophieu = data.sophieu.replace('/','-')
          let routeData =  this.$router.resolve('/nlg-bb-tinh-tien/'+guid.replace('/','-'))
          window.open(routeData.href, '_blank');
        },
    }
}
