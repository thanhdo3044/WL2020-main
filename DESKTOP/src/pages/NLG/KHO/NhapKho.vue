    ...
    <q-card class="my-card">
      ...
      <q-card-section>
       ...
       />
              ...
              <DxColumn data-field="MANCC" caption="Tên Nhà cung cấp" width="350"  >
                  <DxLookup
                    :data-source="dsNcc"
                    display-expr="name"
                   value-expr="code"
              ...
              <template #edit-template="cell">
                <div>
                  <span
                    v-if="cell.data.data.ALLOWMODIFY.toString().trim() === 'Y'"
...
export default {
  ...
  created() {
    ...
    if (this.userInfo.id == 102089) this.sua = true
    if (this.userInfo.factoryId == 100000) this.f_kho = 'TH'
    if (this.userInfo.factoryId == 100003) this.f_kho = 'YS1'
     if (this.userInfo.factoryId == 100004) this.f_kho = 'YS1'
 ...
 this.f_kho = 'Phòng NLG'
  ...
  methods: {
    async save() {
      ...
      if (this.f_ncc) {
        const payload = {
          ...
          createDate: formatDateISO(this.f_ngaynhapkho)+ " 06:00:00",
          makho: this.f_kho,
          MANCC: this.f_ncc
        };
    ...
    // async taophieu(){
    //     const data = await this.$store.dispatch('nlg/',{kho:'all'})
    //    this.dataSource = data.data
