    ...
    <q-card class="my-card">
      ...
      <q-card-section>
        <!-- <div class="row q-mb-sm">
          <div class="col-4">
            <SelectBoxKho :dsKho="dskhos" :onChange="eventChonKho" />
          </div>
          <div class="col-4"></div>
        </div> -->
        <DxDataGrid
...
export default {
  ...
  methods: {
    ...
    async loadDelai() {
      ...
      const data = await this.$store.dispatch("nlg/GET_DS_TRAVE", this.makho);
    ...
    goAdd(guid) {
      this.$router.push({ path: "/nlg-phieu-trave", query: { guid: guid } });
    },
    eventChonKho(e) {
      this.makho = e.value;
