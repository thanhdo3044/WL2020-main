  ...
  <q-page>
    <q-banner class="bg-primary text-orange">
      <div class="text-h6 orange">
        <span color="orange">Nhập tồn Kho thành phẩm</span>
      </div>
    </q-banner>
    <div>
  ...
  data() {
    return {
      columns: [
        ...
        { name: "length2", label: "length", field: "length" },
        { name: "length2", label: "volumn", field: "volumn" },
        { name: "length2", label: "typeid", field: "typeId" },

        { name: "length2", label: "unitId", field: "unitId" },
        { name: "recipeGroup", label: "recipeGroup", field: "recipeGroup" },
        { name: "recipeBirch", label: "recipeBirch", field: "recipeBirch" },
        { name: "recipeLVL", label: "recipeLVL", field: "recipeLVL" },
    ...
    async onClick() {
      if (this.selected) {
        if (this.resultData.length > 0) {
          let result = [];
          let IMPORT_TON_DAU_KY = this.IMPORT_ITEMS;
          ...
          await results.forEach((e) => {
            if (e.name) {
              this.resultData.push({
                ...
                volumn: e.volumn ? e.volumn : null,

                unitId: e.unitId ? e.unitId : null,
                typeId: e.typeId ? e.typeId : null,
                factoryId: e.factoryId ? e.factoryId : null,
                recipeGroup : e.recipeGroup ? e.recipeGroup : null,
                recipeBirch : e.recipeBirch ? e.recipeBirch : null,
                recipeLVL : e.recipeLVL ? e.recipeLVL : null,
              });
            }
          });
