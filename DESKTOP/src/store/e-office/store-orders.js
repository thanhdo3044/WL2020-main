...
const actions = {
    importPaySlip: async ({ commit }, payload) => {
        let data = await client.import("/api/v2/payslip/import", payload);
        console.log(data)   
        return data
    },
    getByCondition: async ({ commit }, payload) => {
        let data = await client.get(`/api/v2/getByCondition/${payload.month}/${payload.year}/${payload.khoi}`);
        console.log(data)   
        return data
    },
    ...
    addOrder: async({ commit }, order) => {
        ...
        if (order.id) {
            ...
            console.log(order);
            let data = await client.post("/api/v1/orders/",order);
            console.log(data);
            return data.data;
        }
    },
    ...
    getAllOrder: async({ commit }, payload) => {
        let data = await client.get("/api/v2/orders/getAllOrders?dateStart=" + payload.dateStart + "&dateEnd=" + payload.dateEnd);
        return data.data;
    },
    searchOrder: async({ commit }, payload) => {
        let data = await client.post("/api/v2/orders/getAllOrders?dateStart=" + payload.dateStart + "&dateEnd=" + payload.dateEnd + "&idDepartment=" + payload.idDepartment + "&activeSuccess=" + payload.activeSuccess);
        return data.data;
    },
    getXNVOrders: async({ commit }, payload) => {
        let data = await client.get("/api/v2/orders/getXNVOrders?dateStart=" + payload.dateStart + "&dateEnd=" + payload.dateEnd);
        return data.data;
    },
    getLTGOrders: async({ commit }, payload) => {
        let data = await client.get("/api/v2/orders/getLTGOrders?dateStart=" + payload.dateStart + "&dateEnd=" + payload.dateEnd);
        return data.data;
    },
    getXRCOrders: async({ commit }, payload) => {
        let data = await client.get("/api/v2/orders/getXRCOrders?dateStart=" + payload.dateStart + "&dateEnd=" + payload.dateEnd);
        return data.data;
    },
