...
planNLG.get('/', (req, res) => {
    ...
    new mssql.Request()
        .query(` SELECT PL.ID ID, PR.NAME NAME, PL.PLANQTY PLANQTY, G.NAME NHOM
    ,PL.GROUP_CODE GROUP_CODE,PL.MANCC mancc,PL.KHO KHO,PL.VUOTNCC

    FROM nlg.PLAN_NLG PL
    JOIN nlg.GROUP_CODE G on G.ID = PL.GROUP_CODE
    JOIN nlg.PROVIDERS PR on PR.CODE = PL.MANCC
                 where PL.DEL_FLAG='N' and YEAR(PL.CREATED_AT) = DATEPART(year, GETDATE()) and MONTH(PL.CREATED_AT) = '${req.query.thang}'
             `, (err, record) => {
            if (err)
