...
productionOrder.get('/:factoryId/ke-hoach-san-xuat-con', async(req, res) => {
  ...
  select number, ROUND( AVG(tiLeTH)*100,1) avgTiLeTH from (
    select number, case when v.soLuongDat is null then 0 else v.soLuongDat end / (PO.keHoach-PO.hanMucTon) as tiLeTH from prod.PO
    left join View_DatKeHoach v on PO.code = v.poCode
      where po.stepId in (
        100270,100273,100282
      ) and po.approvedAt is not null and po.deletedAt is null and po.endPO = 0 and factoryId = @factoryId and PO.keHoach-PO.hanMucTon > 0
      and number in (
          select distinct number from prod.PO po
          left join View_DatKeHoach v on po.code = v.poCode
          where po.stepId in (
            100270,100273,100282
          ) and v.soLuongDat is null or (po.approvedAt is not null and po.deletedAt is null and po.endPO = 0 and factoryId = @factoryId and keHoach > v.soLuongDat)
      )
    ) x
  group by number
  order by number
...
productionOrder.put('/:code', async(req, res) => {
    ...
    const { data } = req.body
    console.log(data)
    const { ys1a, ys1b, ys4, soLuongUuTien } = req.body.data
  ...
  GO;
    ...
    if (ys4 !== undefined) {
        console.log('So luong nhap ve tu YS4:', ys1a)
        const po = await Po.findOne({
            where: {
                code
            }
        })
        const quantity = ys4 - po.ys4
        const congDoanDuois = await new mssql.Request().query(`
                WITH temp0(itemId, stepId, seq)
                    as(
                        select ITEM_ID as itemId, STEP_ID as stepId, [ORDER] as seq from prod.ROUTING where STEP_ID = ${po.stepId}
        ...
        from (
          ...
          UNION ALL
            ...
            from prod.MATERIALS_IN_PACKAGE m
              ...
              left join prod.PACKAGE p on iip.PACKAGE_ID = p.ID
              left join prod.PO po on po.code = p.PO
            where SOURCE_ID = @FROM_ID and p.PO is not null and p.PO in (select code from prod.PO where deletedAt is null and endPo = 0 and approvedAt is not null) --and (po.number = @LENHSX or @FROM_ID in (select ID from base.DEPARTMENT where NAME like N'%đóng gói%' and TYPE2 = 'department'))
            group by m.ITEM_ID
        ) as x
        group by ITEM_ID
  ) ton
...
productionOrder.get('/:stepId/nhan-phoi-nhap-ve', async(req, res) => {
    ...
    select po.*, op.conTon, i.CODE itemCode, i.NAME itemName, i.WIDTH itemWidth, i.HEIGHT itemHeight, i.LENGTH itemLength, case when po.soLuong <= op.conTon then po.soLuong else op.conTon end as nhanVe from (
      select code, itemId, loiCongDon + ys1a + ys1b + ys4 + th + soLuongUuTien - ton as soLuong from prod.PO where stepId = @stepId and deletedAt is null and approvedAt is not null and endPO = 0 and ton < ys1a + ys1b + ys4 + th + soLuongUuTien
...
productionOrder.get('/:factoryId/ke-hoach-san-xuat-cong-doan', async(req, res) => {
    const data = await new mssql.Request().input('factoryId', req.params.factoryId).query(`
  select x.*, x.keHoach - x.soLuongUuTien - x.ys1a - x.ys1b - x.ys4 + x.hanMucTon as canThucHien,x.keHoach + x.hanMucTon - x.soLuongUuTien as conThucHien, i.NAME itemName, i.WIDTH itemWidth, i.HEIGHT itemHeight, i.LENGTH itemLength, d.NAME stepName from (
    select * from prod.PO where approvedAt is null and deletedAt is null and endPo = 0 and factoryId = @factoryId
  ) x
...
productionOrder.get('/:factoryId/kh-ngoai', async(req, res) => {
    const data = await new mssql.Request().input('factoryId', req.params.factoryId).query(`
    select P.nguonPhoi nhamay, I.NAME itemName,D.NAME stepName,sum(P.QUANTITY) nhaptt, sum(ys4) kehoach, SUM(P.QUANTITY - ys4) tonconlai   FROM [Woodsland].[prod].[PO] po
    left join [base].[ITEM] I on I.ID = po.itemId
    left join [base].[DEPARTMENT] D on D.ID = po.stepId
    left join [prod].[OPENING_STOCK] P ON P.DEPARTMENT_ID = po.stepId and P.ITEM_ID = po.itemId
    where (ys4 > 0) and P.QUANTITY > 0 and P.nguonPhoi like '%4'
    GROUP BY 
    I.NAME,D.NAME, P.nguonPhoi
    union 
    select P.nguonPhoi nhamay, I.NAME itemName,D.NAME stepName,sum(P.QUANTITY) nhaptt, sum(ys1a) kehoach, SUM(P.QUANTITY - ys1a) tonconlai   FROM [Woodsland].[prod].[PO] po
    left join [base].[ITEM] I on I.ID = po.itemId
    left join [base].[DEPARTMENT] D on D.ID = po.stepId
    left join [prod].[OPENING_STOCK] P ON P.DEPARTMENT_ID = po.stepId and P.ITEM_ID = po.itemId
    where (ys1a > 0) and P.QUANTITY > 0 and P.nguonPhoi like '%1'
    GROUP BY 
    I.NAME,D.NAME, P.nguonPhoi
`)
    res.status(200).send(data.recordset)
...
productionOrder.get('/number-steps', (req, res, next) => {
    new mssql.Request()
      ...
      WHERE p.status is null and p.endPO = 0 and p.approvedAt is not null and deletedAt is null and soLuongUuTien + loiCongDon > ton
...
productionOrder.get('/:factoryId/kh1-ngoai', async(req, res) => {

    const keHoachThucHien = await new mssql.Request()

    .query(`
    select P.nguonPhoi nhamay, I.NAME itemName,D.NAME stepName,sum(P.QUANTITY) nhaptt, sum(ys4) kehoach  FROM [Woodsland].[prod].[PO] po
    left join [base].[ITEM] I on I.ID = po.itemId
    left join [base].[DEPARTMENT] D on D.ID = po.stepId
    left join [prod].[OPENING_STOCK] P ON P.DEPARTMENT_ID = po.stepId and P.ITEM_ID = po.itemId
    where (ys4 > 0) and P.QUANTITY > 0 and P.nguonPhoi like '%4'
...
productionOrder.get('/:stepId/dinh-muc', async(req, res) => {
    const keHoachThucHien = await new mssql.Request()
        .input('stepId', req.params.stepId)
        .query(`
        select ID id, ITEM_ID itemId, MATERIALS_ID materialId  from prod.BOM
    `)
    res.status(200).send(keHoachThucHien.recordset)
})
...
productionOrder.get('/:code/thuc-hien', async(req, res) => {
    const data = await new mssql.Request()
        ...
        .query(
            `
            select *, x.quantity - x.loiCongDon as sanLuongTruLoi, x.ton - x.xuatTon as conTon, x.keHoach - x.soLuongUuTien - x.ys1a - x.ys1b - x.ys4 + x.hanMucTon + x.loiCongDon - x.quantity + x.ton as conThucHien from (
