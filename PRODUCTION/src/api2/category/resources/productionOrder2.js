...
productionOrder.get('/:factoryId/kh-ngoai', async(req, res) => {
    const data = await new mssql.Request().input('factoryId', req.params.factoryId).query(`
    select  'yên sơn 4' nhamay,I.NAME itemName,D.NAME stepName, sum(P.QUANTITY) nhaptt,sum(ys4) kehoach FROM [Woodsland].[prod].[PO] po
    left join [base].[ITEM] I on I.ID = po.itemId
    left join [base].[DEPARTMENT] D on D.ID = po.stepId
   left join (select DEPARTMENT_ID, ITEM_ID,QUANTITY from [prod].[OPENING_STOCK] where QUANTITY > 0 and nguonPhoi like '%4') P ON P.DEPARTMENT_ID = po.stepId and P.ITEM_ID = po.itemId
    where (ys4 > 0) --and P.QUANTITY > 0 --and P.nguonPhoi like '%4'
    GROUP BY 
    I.NAME,D.NAME
    union
    select  'yên sơn 1' nhamay,I.NAME itemName,D.NAME stepName, sum(P.QUANTITY) nhaptt,sum(ys1a) kehoach FROM [Woodsland].[prod].[PO] po
    left join [base].[ITEM] I on I.ID = po.itemId
    left join [base].[DEPARTMENT] D on D.ID = po.stepId
   left join (select DEPARTMENT_ID, ITEM_ID,QUANTITY from [prod].[OPENING_STOCK] where QUANTITY > 0 and nguonPhoi like '%1') P ON P.DEPARTMENT_ID = po.stepId and P.ITEM_ID = po.itemId
    where (ys1a > 0) --and P.QUANTITY > 0 --and P.nguonPhoi like '%4'
    GROUP BY 
    I.NAME,D.NAME
`)
    res.status(200).send(data.recordset)
