...
productionOrder.get('/:factoryId/ke-hoach-san-xuat-con', async(req, res) => {
  ...
  update prod.PO set ton = keHoach, ys4 = keHoach where code = '1c4dcb09-daa8-41d2-accf-22a64a8071d8'
...
productionOrder.get('/:stepId/ton-tren-chuyen', async(req, res, next) => {
    ...
    const map = new Map()
    const item = { tonDauKy: 0, loiDauKy: 0, nhap: 0, xuat: 0, loi: 0, tonCuoiKy: 0, stepId }
    ...
    await tonDauKys.forEach(t => {
        if (map.has(t.itemId)) {
            ...
            map.set(t.itemId, {...item, market: t.market, itemId: t.itemId, tonDauKy: t.nhap })
        }
    })
    ...
    const data = req.data
        .map(t => {
            const { itemId, tonDauKy, loiDauKy, nhap, xuat, loi } = t
            // console.log(tonDauKy, loiDauKy, nhap, xuat, loi)
            const item = items.find(i => i.id == itemId)
            const p = items.find(i => i.id == t.market)
            return {...t, ...item, market: p.name, tonCuoiKy: nhap > 0 ? tonDauKy - loiDauKy + nhap - xuat - loi : 0 }
        })
...
productionOrder.get('/:stepId/nhan-phoi-nhap-ve', async(req, res) => {
    ...
    select po.*, op.conTon, i.CODE itemCode, i.NAME itemName, i.WIDTH itemWidth, i.HEIGHT itemHeight, i.LENGTH itemLength, case when po.soLuong <= op.conTon then po.soLuong else op.conTon end as nhanVe from (
      select code, number, itemId, loiCongDon + ys1a + ys1b + ys4 + th + soLuongUuTien - ton as soLuong from prod.PO where stepId = @stepId and deletedAt is null and approvedAt is not null and endPO = 0 and ton < ys1a + ys1b + ys4 + th + soLuongUuTien
...
productionOrder.post('/:code/:code1/number-steps', (req, res, next) => {
    ...
    FROM            prod.PO AS p 
    WHERE p.status is null and p.endPO = 0 and p.approvedAt is not null and deletedAt is null 
    and p.stepId = '${req.params.code}'  and itemId = '${req.params.code1}' 
    ORDER BY p.approvedAt 
    `,
            (err, record) => {
                if (err) {
                    console.log(err)
...
productionOrder.get('/:factoryId/rp-thuc-hien-san-xuat', async(req, res) => {
  ...
  select d.NAME stepName, i.NAME itemName, i.WIDTH itemWidth, i.HEIGHT itemHeight, i.LENGTH itemLength, t.*, v.soLuongDat, thucHien - case when v.soLuongDat is null then 0 else v.soLuongDat end + (ton - xuatTon) as conThucHien from (
    select id, code, number, itemId, stepId, keHoach, soLuongUuTien + ys1a + ys1b + ys4 + th as soLuongUuTien, keHoach + hanMucTon - soLuongUuTien - ys1a - ys1b - ys4 - th as thuanHung, loiCongDon, ton, xuatTon, ton - xuatTon as coTheXuat, keHoach + hanMucTon - soLuongUuTien - ys1a - ys1b - ys4 -th + xuatTon + loiCongDon as thucHien from prod.PO where deletedAt is null and approvedAt is not null and endPO = 0 and factoryId = ${req.params.factoryId} --and number = N'TH-W47-2020'
    ) t
    left join View_DatKeHoach v on v.poCode = t.code
      left join base.DEPARTMENT d on d.ID = t.stepId
      left join base.ITEM i on i.ID = t.itemId
...
select * from (
  select
                     ...
                     CASE
                         ...
                         ELSE ROUND(SL.quantity,6)
                     END AS 'totalQuantity',
                     ROUND(PO.keHoach - PO.soLuongUuTien + PO.hanMucTon + PO.loiCongDon-(CASE
                         WHEN SL.quantity IS NULL THEN 0
                         ELSE ROUND(SL.quantity,6)
                     END),6) conThucHien,
                     SLN.quantity quantityByDay,
...
SELECT * FROM View_ROUTINGD where factoryId = @factoryId
`
